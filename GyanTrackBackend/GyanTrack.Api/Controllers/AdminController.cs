...
namespace GyanTrack.Api.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        /// <summary>
        /// GET: api/admin/evaluations/pending
        /// Get all evaluations that are verified by evaluators but pending HR final score
        /// </summary>
        /// <returns>List of pending verified test attempts</returns>
        [HttpGet("evaluations/pending")]
        public async Task<ActionResult<IEnumerable<TestAttemptDTO>>> GetPendingEvaluations()
        {
            try
            {
                var attempts = await _assessmentService.GetPendingHRVerificationsAsync();
                return Ok(attempts);
            }
            catch (Exception ex)
            {
